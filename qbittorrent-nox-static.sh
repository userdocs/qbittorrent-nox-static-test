...
_set_default_values() {
	...
	qbt_mcm_repo="${qbt_mcm_repo:-userdocs/qbt-musl-cross-make}"
	...
	if [[ "${multi_arch_options[${qbt_cross_name:-default}]}" == "${qbt_cross_name}" ]]; then
		if [[ "${os_id}" =~ ^(alpine|debian|ubuntu)$ ]]; then
			...
			if [[ "${qbt_cross_target}" =~ ^(alpine)$ ]]; then
				if [[ "${1}" == 'bootstrap' || "${qbt_cache_dir_options}" == "bs" || ! -f "${qbt_cache_dir:-${qbt_install_dir}}/${qbt_cross_host}.tar.gz" ]]; then
					printf '\n%b\n' " ${unicode_blue_light_circle} Downloading ${color_magenta_light}${qbt_cross_host}.tar.gz${color_end} cross tool chain - ${color_cyan_light}https://github.com/${qbt_mcm_repo}/releases/latest/download/${qbt_mcm_toolchain_prefix}-${qbt_cross_host}.tar.xz${color_end}"
					_curl --create-dirs "https://github.com/${qbt_mcm_repo}/releases/latest/download/${qbt_mcm_toolchain_prefix}-${qbt_cross_host}.tar.xz" -o "${qbt_cache_dir:-${qbt_install_dir}}/${qbt_cross_host}.tar.gz"
				fi
